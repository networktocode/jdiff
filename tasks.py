...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_JDIFF_xxx
namespace = Collection("jdiff")
namespace.configure(
    {
        "jdiff": {
            "project_name": "jdiff",
...
def run_command(context, exec_cmd, port=None, rm=True):
    ...
    """
    if is_truthy(context.jdiff.local):
        ...
        if port:
            result = context.run(
                f"docker run -it {'--rm' if rm else ''} -p {port} -v {context.jdiff.pwd}:/local {context.jdiff.image_name}:{context.jdiff.image_ver} sh -c '{exec_cmd}'",
                pty=True,
            )
        else:
            result = context.run(
                f"docker run -it {'--rm' if rm else ''} -v {context.jdiff.pwd}:/local {context.jdiff.image_name}:{context.jdiff.image_ver} sh -c '{exec_cmd}'",
                pty=True,
            )
...
def build(context, cache=True, force_rm=False, hide=False):
    ...
    print(f"Building image {context.jdiff.image_name}:{context.jdiff.image_ver}")
    command = f"docker build --tag {context.jdiff.image_name}:{context.jdiff.image_ver} --build-arg PYTHON_VER={context.jdiff.python_ver} -f Dockerfile ."
...
def coverage(context):
    ...
    exec_cmd = "coverage run --source=jdiff -m pytest"
...
def pytest(context, pattern=None, label=None):
    ...
    exec_cmd = "pytest -vv --doctest-modules jdiff/ && coverage run --source=jdiff -m pytest && coverage report"
    run_command(context, exec_cmd)

    doc_test_cmd = "pytest -vv --doctest-modules jdiff/"
    pytest_cmd = "coverage run --source=jdiff -m pytest"
    if pattern:
...
def cli(context):
    ...
    Args:
        context (obj): Used to run specific commands
    """
    dev = f"docker run -it -v {context.jdiff.pwd}:/local {context.jdiff.image_name}:{context.jdiff.image_ver} /bin/bash"
